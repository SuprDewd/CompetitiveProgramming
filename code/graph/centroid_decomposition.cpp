...
struct centroid_decomposition {
    ...
    int dfs(int u, int p) {
        sz[u] = 1;
        rep(i,0,size(adj[u])) if (adj[u][i] != p) sz[u] += dfs(adj[u][i], u);
        return sz[u]; }
    void makepaths(int sep, int u, int p, int len) {
        ...
        if (p == sep) swap(adj[u][bad], adj[u].back()), adj[u].pop_back(); }
    ...
    void paint(int u) {
        rep(h,0,seph[u]+1)
            shortest[jmp[u][h]] = min(shortest[jmp[u][h]], path[u][h]); }
    int closest(int u) {
        int mn = INF/2;
        rep(h,0,seph[u]+1) mn = min(mn, path[u][h] + shortest[jmp[u][h]]);
        return mn; } };
