...
ll crt(vector<ll> &as, vector<ll> &ns) {
    ll cnt = size(as), N = 1, x = 0, r, s, l; rep(i,0,cnt) N *= ns[i];
...
pair<ll,ll> gcrt(vector<ll> &as, vector<ll> &ns) {
    ...
    rep(at,0,size(as)) {
        ...
        if (n > 1 && n > ms[n].first) ms[n] = make_pair(n, as[at] % n); }
    vector<ll> as2, ns2; ll n = 1;
    iter(it,ms) {
        as2.push_back(it->second.second); ns2.push_back(it->second.first);
        n *= it->second.first; }
    ll x = crt(as2,ns2);
