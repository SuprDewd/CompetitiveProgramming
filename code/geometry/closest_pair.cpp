...
struct cmpx { bool operator ()(const point &a, const point &b) {
        return abs(real(a) - real(b)) > EPS ?
            real(a) < real(b) : imag(a) < imag(b); } };
struct cmpy { bool operator ()(const point &a, const point &b) {
...
double closest_pair(vector<point> pts) {
    ...
    for (int i = 0, l = 0; i < size(pts); i++) {
        while (real(pts[i]) - real(pts[l]) > mn) cur.erase(pts[l++]);
